...
variable "secondary_location" {
  description = "Azure region for the failover group and read-only replicas."
  type        = string

  validation {
    condition     = var.secondary_location != null
    error_message = "Secondary location must be specified for failover group and replication."
  }
}
...
variable "resource_group_name" {
  description = "The name of the resource group to deploy MSSQL resources into."
  type        = string
}
...
variable "databases" {
  description = "A map of databases to be created with replication and failover settings."
  type = map(object({
    name                      = string
    ...
    read_replica_count        = optional(number, 2)
  ...
  validation {
    condition     = alltrue([for db in var.databases : db.failover_group_enabled ? db.secondary_location != null : true])
    error_message = "Failover group requires a secondary location to be specified in each database entry."
  }
}
